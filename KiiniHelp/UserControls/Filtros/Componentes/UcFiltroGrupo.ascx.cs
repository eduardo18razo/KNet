...
namespace KiiniHelp.UserControls.Filtros.Componentes
{
    public partial class UcFiltroGrupo : UserControl, IControllerModal
    {
        ...
        private void LlenaGrupos()
        {
            try
            {
                rptGpos.DataSource = _servicioGrupoUsuario.ObtenerGruposByIdUsuario(((Usuario)Session["UserData"]).Id, false).Where(w=>w.IdTipoGrupo != (int)BusinessVariables.EnumTiposGrupos.Acceso);
                rptGpos.DataBind();
            }
            catch (Exception e)
            {

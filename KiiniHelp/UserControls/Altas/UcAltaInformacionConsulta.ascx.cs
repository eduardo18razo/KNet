        ...
        protected void afuArchivo_OnUploadedComplete(object sender, AsyncFileUploadEventArgs e)
        {
            try
            {
                ...
                if (!files.Any(a => a.NombreArchivo.Contains(afuArchivo.FileName)))
                {
                    ...
                    HelperFiles hFiles = new HelperFiles { NombreArchivo = e.FileName, Tamaño = BusinessFile.ConvertirTamaño(e.FileSize), Extension = Path.GetExtension(afuArchivo.FileName) };
                    files.Add(hFiles);

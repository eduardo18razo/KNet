...
{
    public partial class FrmServicios : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<ArbolAcceso> lstArboles = _servicioArbolAcceso.ObtenerArblodesAccesoByGruposUsuario(((Usuario)Session["UserData"]).Id, (int)BusinessVariables.EnumTipoArbol.Servicio).Distinct().ToList();
        ...
        protected void tvArbol_OnSelectedNodeChanged(object sender, EventArgs e)
        {
            try
            {
                if (((TreeView)sender).SelectedNode.Target != string.Empty)
                    Response.Redirect("~/Ticket/FrmTicket.aspx?IdArbol=" + ((TreeView)sender).SelectedNode.Target);
            }
            catch (Exception)
            {
